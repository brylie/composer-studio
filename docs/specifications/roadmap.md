...
| [timeline.md](./timeline.md) — event-track infra                                                                         | Medium                                                                                                                             | Medium                                                                                               | Nothing new                                                                      | tracks.md (time signature/scale/chord/labels)                                                                             |
| [tracks.md](./tracks.md) — time signature track                                                                          | Medium — a cross-cutting concern (grid rendering, snap-grid interaction) even though nothing in the priority use case blocks on it | Low-Medium (preset picker + lane/editor reuse; beat-grouping grid lines are the only new rendering)  | timeline.md                                                                      | Editable, mid-piece time-signature changes with correct bar/beat-grouping rendering; v2 unlocks additive/irregular meters |
...
### Phase 6 — Timeline infrastructure + Time signature + Scale track

[timeline.md](./timeline.md), [tracks.md](./tracks.md) (time signature and
scale tracks).

The generic `EventTrack<T>`/`activeEventAt` abstraction; the
[time signature track](./tracks.md#time-signature-track-specified) — preset
picker, bar-line and beat-grouping grid rendering, and its (deliberately
unchanged) relationship to the snap grid; and the scale track's context-aware
highlighting — the trickiest UI spec in the whole set (per-segment bands, not
a single global highlight set). This unlocks scale-aware transforms
(`mode-shift`, `reharmonization`, which need
`SelectionContext.activeScales`) and `generate-chords`'s `source:
'chord-track'` path as an upgrade over `selection-derived`. Deliberately
after Phase 2–5: nothing in the priority use case blocks on it.
