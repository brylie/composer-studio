...
them — the button would only happen to update when _something else_ also
...
_and_ a scale event in the same user action (rare, but possible for something
...
This keeps `CommandHistory` ignorant of _what_ a document is — it just stores
