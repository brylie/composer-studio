...
| [direct-manipulation.md](./direct-manipulation.md) | Quick-apply: running a command/generator straight on an existing selection, on-canvas param affordances instead of drawers, semi-live tweak-then-bake                         |
| [unified-operations.md](./unified-operations.md)   | Collapsing the command/generator UI split into one discovery contract and interaction model, so intuitions transfer between transforms and generators instead of diverging    |
