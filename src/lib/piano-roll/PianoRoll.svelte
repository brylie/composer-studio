<script lang="ts">
	...
	import { NOTE_COUNT, MAX_MIDI } from './types.js';
	...
	$effect(() => {
		if (!scrollEl) return;
		const MIDI_C4 = 60;
		const c4Row = MAX_MIDI - MIDI_C4; // row index from top
		const targetY = c4Row * store.rowHeight - scrollEl.clientHeight * 0.33;
		scrollEl.scrollTop = Math.max(0, targetY);
	});

	// Auto-scroll to keep playhead visible during playback
	$effect(() => {
		if (!store.isPlaying || !scrollEl) return;
		const playheadX = store.currentBeat * store.pixelsPerBeat;
		const viewLeft = scrollEl.scrollLeft;
		const viewRight = viewLeft + scrollEl.clientWidth - 64; // subtract piano keys width
		if (playheadX > viewRight - 120) {
			scrollEl.scrollLeft = playheadX - 120;
		}
	});
	...
	// Sync velocity lane with main scroll area
	$effect(() => {
		if (!scrollEl) return;
		function syncVel() {
			velScrollLeft = scrollEl!.scrollLeft;
		}
		scrollEl.addEventListener('scroll', syncVel, { passive: true });
		return () => scrollEl!.removeEventListener('scroll', syncVel);
	});
			...
			<div class="scroll-area" bind:this={scrollEl}>
				<div class="scroll-content" style="width: max-content; min-height: 100%;">
					...
					<div class="ruler" style="width: {rulerWidth}px; position: sticky; top: 0; z-index: 20;">
						{#each Array.from({ length: barCount }, (_, i) => i) as bar}
